...
import { getLocaleLabel, getLocalizedHomePath, isSupportedLocale, supportedLocales, type Locale } from "@/lib/i18n";
...
function buildLanguageOptions(locale: Locale) {
  return supportedLocales.map((entry) => ({
    locale: entry,
    label: getLocaleLabel(entry),
    href: getLocalizedHomePath(entry),
    active: entry === locale,
  }));
}
...
export async function generateMetadata({ params }: LocalizedHomePageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: getLocalizedHomePath("en"),
        es: getLocalizedHomePath("es"),
      },
    },
  };
}
...
export default async function LocalizedHomePage({ params }: LocalizedHomePageProps) {
  ...
  const config = getClientConfig(locale);
  const { businessName, contact, contactSection, cta, footer, form, gallery, header, hero, packages, services, trust, whatsapp, whyChooseUs } = config;

  return (
    <main className="page-stack flex-1 bg-(--page-background) text-(--text-primary)">
      <Navbar header={header} languageOptions={buildLanguageOptions(locale)} />
      ...
      <Gallery section={gallery} locale={locale} />
